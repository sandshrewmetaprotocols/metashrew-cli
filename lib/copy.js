...
const maximatch_1 = __importDefault(require("maximatch"));
const errno_1 = __importDefault(require("errno"));
const path_1 = __importDefault(require("path"));
const events_1 = __importDefault(require("events"));
const logger = (0, logger_1.getLogger)();
var Promise = global.Promise || require("promise");
var CopyError = errno_1.default.custom.createError("CopyError");
...
const mkdir = mkdirp_1.mkdirp;
const { stat, lstat, readlink, symlink, readdir } = fs_extra_1.default;
const remove = rimraf_1.rimraf;
function copyRecursive(src, dest, options, callback) {
    if (arguments.length === 3 && typeof options === "function") {
        callback = options;
        options = undefined;
    }
    options = options || {};
    var parentDirectory = path_1.default.dirname(dest);
    ...
    var promise = ensureDirectoryExists(parentDirectory)
        ...
        var relativePaths = filePaths.map(function (filePath) {
            return '.' + path_1.default.sep + path_1.default.relative(src, filePath);
        });
        ...
        return filteredPaths.map(function (relativePath) {
            ...
            return {
                src: path_1.default.join(src, inputPath),
                dest: path_1.default.join(dest, outputPath),
            };
        });
    })
        ...
        var hasFinishedGetter = function () {
            return hasFinished;
        };
...
function getFilePaths(src, shouldExpandSymlinks) {
    return (shouldExpandSymlinks ? stat : lstat)(src).then(function (stats) {
        if (stats.isDirectory()) {
            return getFileListing(src, shouldExpandSymlinks).then(function (filenames) {
...
function getFilteredPaths(paths, filter, options) {
    ...
    return paths.filter(function (path) {
        return ((!useDotFilter || dotFilter(path)) &&
            (!useJunkFilter || junkFilter(path)) &&
            (!filter || (0, maximatch_1.default)((0, slash_1.default)(path), filter, options).length > 0));
    });
}
...
function junkFilter(relativePath) {
    var filename = path_1.default.basename(relativePath);
    return !(0, junk_1.isJunk)(filename);
}
function ensureDirectoryExists(path) {
    return mkdir(path);
}
function getFileListing(srcPath, shouldExpandSymlinks) {
    return readdir(srcPath).then(function (filenames) {
        return Promise.all(filenames.map(function (filename) {
            var filePath = path_1.default.join(srcPath, filename);
            return (shouldExpandSymlinks ? stat : lstat)(filePath).then(function (stats) {
                if (stats.isDirectory()) {
                    return getFileListing(filePath, shouldExpandSymlinks).then(function (childPaths) {
...
function copy(srcPath, destPath, hasFinished, emitEvent, options) {
    return prepareForCopy(srcPath, destPath, options)
        ...
        .then(function (result) {
        logger.star(path_1.default.relative(process.cwd(), destPath));
        return result;
    });
}
function prepareForCopy(srcPath, destPath, options) {
    ...
    return (shouldExpandSymlinks ? stat : lstat)(srcPath).then(function (stats) {
        return ensureDestinationIsWritable(destPath, stats, shouldOverwriteExistingFiles).then(function () {
            return stats;
        });
    });
}
...
function createCopyFunction(fn, stats, hasFinished, emitEvent, events) {
    ...
    return function (srcPath, destPath, stats, options) {
        ...
        var parentDirectory = path_1.default.dirname(destPath);
...
function copyFile(srcPath, destPath, stats, options) {
    return new Promise(function (resolve, reject) {
        var hasFinished = false;
        var read = fs_extra_1.default.createReadStream(srcPath);
        read.on("error", handleCopyFailed);
        var write = fs_extra_1.default.createWriteStream(destPath, {
            flags: "w",
            mode: stats.mode,
        });
        write.on("error", handleCopyFailed);
        write.on("finish", function () {
            fs_extra_1.default.utimes(destPath, stats.atime, stats.mtime, function () {
                hasFinished = true;
                resolve();
            });
        });
...
function copySymlink(srcPath, destPath, stats, options) {
    return readlink(srcPath).then(function (link) {
        return symlink(link, destPath);
    });
}
function copyDirectory(srcPath, destPath, stats, options) {
    return mkdir(destPath).catch(function (error) {
...
function fsError(code, path) {
    var errorType = errno_1.default.code[code];
...
function withEventEmitter(target) {
    for (var key in events_1.default.prototype) {
        target[key] = events_1.default.prototype[key];
    }
    events_1.default.call(target);
    return target;
}
exports.events = {
    ERROR: EVENT_ERROR,
