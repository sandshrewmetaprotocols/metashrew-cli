...
const mkdir = mkdirp;
const { stat, lstat, readlink, symlink, readdir } = fs;
const remove = rimraf;

export default function copyRecursive(src, dest, options?, callback?) {
...
function getFilePaths(src, shouldExpandSymlinks) {
  return ((shouldExpandSymlinks ? stat : lstat) as any)(src).then(
    function (stats) {
      if (stats.isDirectory()) {
        return getFileListing(src, shouldExpandSymlinks).then(
          function (filenames) {
...
function getFilteredPaths(paths, filter, options) {
  ...
  return paths.filter(function (path) {
    return (
      ...
      (!filter || maximatch(slash(path), filter, options).length > 0)
    );
  });
}
  ...
  return readdir(srcPath).then(function (filenames) {
    return Promise.all(
      filenames.map(function (filename) {
        ...
        return (shouldExpandSymlinks ? stat : lstat)(filePath).then(
          function (stats) {
            if (stats.isDirectory()) {
              return getFileListing(filePath, shouldExpandSymlinks).then(
                function (childPaths) {
...
function prepareForCopy(srcPath, destPath, options) {
  ...
  return (shouldExpandSymlinks ? stat : lstat)(srcPath).then(function (stats) {
    return ensureDestinationIsWritable(
      destPath,
      stats,
...
function ensureDestinationIsWritable(
  destPath,
  srcStats,
  shouldOverwriteExistingFiles,
) {
